...
namespace JG
{
	...
	public class Ray
    {
        ...
        public Ray(dynamic pnt, dynamic dir)
        {									
            P = new Vec();			
			P.add1(pnt);
            D = new Vec();			
	...
	public class RayTracer
    {
		...
		public dynamic createScene()
        {
            ...
            dynamic light = new Light(100.0, 100.0, -50.0, 1.0); 
            scene.addLight(light);			
			light = new Light(-100.0, 100.0, -50.0, 1.0); 
            scene.addLight(light);			
			light = new Light(100.0, -100.0, -50.0, 1.0); 
            scene.addLight(light);			
            light = new Light(-100.0, -100.0, -50.0, 1.0);
			scene.addLight(light);
            light = new Light(200.0, 200.0, 0.0, 1.0);
			scene.addLight(light);
		...
		{			
			...
			dynamic r = new Ray(localView.fromVec, RayTracer.voidVec);
	...
	public class JGFRayTracerBench : RayTracer
    {
        ...
        public void JGFapplication()
        {
			int lWidth = width;
			int lHeight = height;			
            dynamic interval = new Interval(0, lWidth, lHeight, 0, lHeight, 1);
            render(interval);
        }
